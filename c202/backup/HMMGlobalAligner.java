	...
	*/	
	...
	public void printPosteriorProbs() {
		...
		*/	
		...
		//create duplicate RCMSA
		HMMAlignment hA = (HMMAlignment)((HMMAlignment)myHMM).clone();
		try {
			hA.addSequence(curSeq);
		}
		catch (DataFormatException ex) {
			Logger.debugln("Couldn't add new sequence to the alignment object");
		}
	    ...
	    while(currCell != null){
			...
			if(currState == HMM.MATCH){
				...
				if(prevCell.yLoc == currCell.yLoc){
					Logger.debugln("adding insert gapchar at "+(i));
					hA.addInsertAtCol(i);
					Logger.debugln("while y,x = "+currCell.yLoc+","+currCell.xLoc+" pMatch:"+currCell.pMatch);
					output.push(" LETTER:"+sequence.charAt(i)+"\tPROB="+ 
						MathUtil.round( Math.exp( currCell.pMatch - probBaseE ) * 100 ) +
						" x,y: " +currCell.xLoc+","+currCell.yLoc+" pTBMatch: "+currCell.pMatch+
						" probE: "+probBaseE);	
				}
				else{
					Logger.debugln("while y,x = "+currCell.yLoc+","+currCell.xLoc+" pMatch:"+currCell.pMatch);
				...
				if(matrix[i][j] != null &&
		       		matrix[i][j].prevStateToMatch != HMM.BEGIN &&
		       		matrix[i][j].prevStateToMatch != 0){
			...
			if(currState == HMM.DELETION){
				Logger.debugln("adding delete gapchar at "+(j-1));
				hA.addDeleteAtCol(j-1);
				Logger.debugln("while y,x = "+currCell.yLoc+","+currCell.xLoc+" pDel: "+currCell.pDelete );
				output.push(" LETTER:"+Alignment.GAP_CHAR+"\tPROB="+ 
				MathUtil.round( Math.exp( currCell.pDelete - probBaseE ) * 100 ) +
				" x,y: " +currCell.xLoc+","+currCell.yLoc+" dTBMatch: "+currCell.pDelete+
				" probE: "+probBaseE);
				if(matrix[i][j] != null &&
		       		matrix[i][j].prevStateToDelete != HMM.BEGIN &&
		       		matrix[i][j].prevStateToDelete != 0){
					
						Logger.debugln("\t_____iterator____");
						Logger.debugln("\tcurr x,y: "+currCell.xLoc+","+currCell.yLoc+
				       	" deleteTB: "+matrix[i][j].deleteTraceBack.xLoc+","+matrix[i][j].deleteTraceBack.yLoc+
	    ...
	    while (!output.empty()){
			...
			//for(int k=output.size()-1; k>=0; k--){
				Logger.println("COLUMN:"+col+"\t"+output.pop().toString() );				
				col++;
			//}
	    }
		try{
